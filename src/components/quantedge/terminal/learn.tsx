...
// QUANTEDGE PRO — Help / Learn Center (§9): friendly, visual, searchable,
...
const SECTIONS: Section[] = [
  {
    ...
    articles: [
      { q: "What is QuantEdge Pro?", a: "An AI-powered market intelligence and trading terminal. It tells you what is happening in the market, why it is happening, which opportunities and risks exist — and, only if you ask, it can help you act through SENTINEL. Analytics is the primary product; SENTINEL is the optional action layer.", tags: ["basics"] },
      ...
      { q: "What is a Catalyst?", a: "An event that can move a price: earnings, guidance, analyst actions, contracts, regulatory decisions, unusual volume. QuantEdge only shows catalysts from verified providers. If no news provider is connected, the catalyst feed says NEWS DATA UNAVAILABLE — it never invents headlines. Volume-based catalysts keep working from real market data.", tags: ["catalysts"] },
      { q: "What is Market Regime?", a: "QuantEdge's read of overall conditions — Risk-On, Risk-Off, High Volatility, Sideways, Momentum — computed from index trends, breadth, and volatility. The regime adjusts signal thresholds, position sizing, and stop distances, so behavior fits the environment.", tags: ["regime"] },
      { q: "What is Paper Trading?", a: "Simulated trading against delayed market prices with modeled slippage, spread, and latency. Real money is never involved. Orders are clearly labeled 'QuantEdge Simulated'. Connecting Alpaca Paper (BYOK) routes fills to Alpaca's paper endpoint instead.", tags: ["paper"] },
    ],
  },
  {
    id: "analytics",
    ...
    articles: [
      { q: "Understanding VWAP", a: "Volume-Weighted Average Price — the average price weighted by volume through the session. Institutions benchmark against it: trading above VWAP means buyers have paid more as the day progressed (control), below means sellers do. QuantEdge weighs your position vs VWAP at up to 15 points.", tags: ["indicators"] },
      { q: "Understanding EMA", a: "Exponential Moving Average — a trend average that reacts faster to recent prices than a simple average. QuantEdge uses the 20/50 stack: price above both, and the 20 above the 50, is a bullish structure worth up to 18 points.", tags: ["indicators"] },
      { q: "Understanding RSI", a: "Relative Strength Index (0–100) — momentum on a speedometer. 55–72 is the healthy bullish band; above 72 is overbought stretch; below 28 is oversold. QuantEdge rewards the healthy band and discounts extremes.", tags: ["indicators"] },
      ...
      { q: "Understanding portfolio risk", a: "Beyond P&L: sector allocation, concentration (HHI), correlation between holdings, volatility, drawdown, and scenario shocks. QuantEdge explicitly warns when several positions behave like one trade — e.g. NVDA + AMD + SMH.", tags: ["risk"] },
    ],
  },
  {
    id: "sentinel",
    ...
    articles: [
      ...
      { q: "Broker connection", a: "Default is the QuantEdge Simulated paper broker with honest fill modeling. Alpaca Paper is available via BYOK keys (Settings → Broker) — your keys are encrypted server-side, never exposed to the browser, never logged.", tags: ["brokers"] },
    ],
  },
  {
    id: "research",
  ...
  },
  {
    ...
    articles: [
      ...
      { q: "BYOK — bring your own key", a: "Where providers cost money (news APIs, Alpaca), you connect your own keys. Keys are stored encrypted, used server-side only, never rendered in the UI, and never returned by APIs. QuantEdge shows usage metering so you can see exactly what consumed what.", tags: ["providers"] },
...
export function LearnView() {
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Learn center" sub="Understand QuantEdge without leaving the screen" />
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-hairline bg-panel py-3 pl-11 pr-4 text-sm outline-none transition-colors focus:border-pos"
        />
      </div>
      ...
      {filtered.map((section) => (
        <div key={section.id} className="qe-panel overflow-hidden">
          <div className="flex items-center gap-3 border-b border-hairline px-5 py-3.5">
            <BookOpen className="h-4 w-4 text-pos" />
            <div>
