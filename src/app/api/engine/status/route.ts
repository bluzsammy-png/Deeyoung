...
// paper trading engine: account, open positions, recent orders, closed
// trades, book stats, feed venue, brain state. "No lies" rule: this reads
// ONLY real rows from Postgres written by real fills at real market prices.
// No auth (like metaapi-diag): aggregates only, no secrets exist here.

import { NextResponse } from "next/server";
import { db } from "@/lib/db";
import { getOrCreateRun, RUN_LABEL } from "@/lib/engine/paper";
import { feedSource } from "@/lib/engine/feed";
import { twelvedataStatus } from "@/lib/market/twelvedata";

export const dynamic = "force-dynamic";

function bookStats(rows: Array<{ gate: number; horizonMin: number; netPnlUsd: number | null; netR: number | null; grossPnlUsd: number | null }>) {
